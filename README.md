...
```json
{
  "mcpServers": {
    "robot-world-bridge": {
      "command": "python",
      "args": ["/ABS/PATH/TO/mcp_world_server.py", "--stdio"],
      "env": {
...
> Важно: для LM Studio bridge скрипт должен запускаться с флагом `--stdio`, иначе процесс завершится и ты увидишь `MCP error -32000: Connection closed`.
